...
const SignUp = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    // ตรวจสอบรูปแบบอีเมลและเงื่อนไขที่ไม่ต้องการ
    const emailLower = email.toLowerCase();
    const emailRegex = /^[^\s@]+@[^\s@]+\.[^\s@]+$/; // Regex พื้นฐานสำหรับอีเมล

    if (
      !emailRegex.test(email) ||
      (!emailLower.endsWith("@gmail.com") && !emailLower.endsWith("@yahoo.com"))
    ) {
      seterrorLogin(true);
      seterrorLoginData("Wrong Email");
      return;
    }

    // ถ้า email ถูกต้อง ดำเนินการ try-catch
    try {
      ...
      seterrorLoginData("Registration successful");
    ...
    <div className="font-primary max-h-[80%] max-w-screen  flex justify-center items-center bg-linear-to-bl from-[#5085A7] to-[#ABCDD9]">
      <div className="w-full h-screen  flex max-lg:justify-center max-lg:items-center ">
        ...
        <div className="md:w-[70%] w-[85%] flex flex-col justify-center bg-white xl:basis-4/10 lg:basis-5/10 xl:gap-30 lg:gap-25  lg:my-20 lg:mr-20 p-6 rounded   shadow-xl relative  ">
          <NavLink
            to="/"
            className="absolute lg:right-5 md:right-10 right-5 md:top-5 top-5  "
          >
            <IconButton
              type="button"
              aria-label="home"
              sx={{
                "& .MuiSvgIcon-root": { color:"#7F8CAA" , fontSize: { xs: "24px", md: "32px" } },
              }}
            >
              <HomeOutlinedIcon />
            </IconButton>
            ...
            >
              <TextField
                ...
                sx={{
                  ...
                  marginBottom: { lg: "48px", md: "40px", xs: "28px" },
                }}
                label="Email"
                type="text"
                value={email}
              ...
              />
              <div
                className={`lg:my-6 md:my-4 my-4 lg:text-[18px] md:text-[14px] text-[12px] text-red-400 ${errorLogin ? "block" : "opacity-0"}`}
              >
                {errorLoginData}
              </div>
              <Button
              ...
              <div className="flex mx-auto items-center gap-2">
                ...
                <NavLink
                  to="/login"
                  className="xl:text-2xl md:text-xl text-md font-bold"
                >
                  Login
                </NavLink>
              </div>
